...
%
% NLSSOL keywords in optPar(#) - and the fields used in optParam
% DESCRIPTION of optPar vector:
%
% Use missing value (-999 or less), when no change of parameter setting is
% wanted. No call is then done to nlsoli or nlsolr.
% The default value will then be used by NLSSOL, 
...
%     -DerLevel - Is set by nlssolTL dependent on Prob.ConsDiff, Prob.NumDiff
...
%
% NLSSOL creates a print file (print.dat) and a summary file (summary.dat).
%
% -------------------------------------------------------------------------
%
...
% Written Sept 17, 2000.  Last modified Dec 22, 2004.
%
...
% funrdr Name of routine [f,gradf] = funrdr(x, Prob, mode, nstate, misVal)
%        funrdr=nlp_rJ, included in TOMLAB.
...
Prob = iniSolve(Prob,6,Prob.NumDiff~=6,Prob.mNonLin>0 & Prob.ConsDiff~=6);
%Prob = iniSolve(Prob,6,1,1);
...
[optPar, SpecsFile, PrintFile, SummFile] = SOLSet('nlssol',6,...
         n, m2, m1, Prob);

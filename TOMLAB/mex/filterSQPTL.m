...
%
%     optPar(20): Nonlin  0     If 1, skip linear feasiblity tests
...
%      solvers does not differentiate between linear and and nonlinear
...
% Written Dec 13, 2002.   Last modified Dec 2, 2004.
...
fLow = DefPar(Prob.DUNDEE,'QPmin',-1E300);
...
if ~isempty(Prob.USER.g),Result.g_k  = nlp_g(x_k,Prob);end
if ~isempty(Prob.USER.H),Result.H_k  = nlp_H(x_k,Prob);end

Result.v_k  = v_k(n+1:end);
...
% MODIFICATION LOG:
%
% 021223 hkh  Avoid calls to Prob.USER.f etc. directly, could crash

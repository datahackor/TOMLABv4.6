...
%              6 - Memory error
%              7 - Unknown error. Contact support@tomlab.biz
%
...
% Written July 3, 2002.  Last modified Jan 11, 2005.
%
...
if nargin < 1, error('pensdpTL needs the Prob structure as input');return;end

global MAX_x MAX_c MAX_r % Max number of variables/constraints/resids to print

Prob.solvType = 13; % LMI solver

Prob = iniSolve(Prob,13,0,0);

Result=ResultDef(Prob);
Result.Solver='PENSDP';
Result.SolverAlgorithm='LMI Solver PENSDP 2.0';
...
if isempty(p)
   ...
   m   = length(ixU) + length(ixL) + length(ibU) + length(ibL);

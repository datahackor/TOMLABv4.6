...
% 2. Prob.USER.H or Prob.USER.d2c is nonempty, i.e. functions are given
...
% -------------------------------------------------------------------------
%
% Anders Goran, Tomlab Optimization Inc, E-mail: anders.goran@tomlab.biz
% Copyright (c) 2003-2004 by Tomlab Optimization Inc., $Release: 4.5.0$
% Written May 20, 2003.   Last modified Dec 2, 2004.
...
LS2PTJ  = DefPar(options,'LS2PTJ',0);

if LS2PTJ >= 1
   % Internal estimation of 2nd order information by numerical differences
   Prob = iniSolve(Prob,3,1,1);
elseif Prob.NumDiff < 0 | Prob.ConsDiff < 0 | ...
   ~isempty(Prob.USER.H) | ~isempty(Prob.USER.d2c)
...
for i=1:m1
    ...
    if abs(bl(i)-bu(i)) < 1e-8       % Equality ? 
...
for i=1:m2
   if abs(cl(i)-cu(i)) < 1e-8  % Equality
      ctype  = [ctype;0];
      c      = [c;cl(i)];
...
% PrintFile  = DefPar(CONOPT,'PrintFile','conoptout.txt');
% StatFile   = DefPar(CONOPT,'StatFile','conoptstat.txt');
...
if m2>0
    Result.c_k = yval(m1ex+1:m1ex+m2);
else
    Result.c_k = [];
end
...
% 041130 ango Minor fix: Prob.USER.d2c, not d2L

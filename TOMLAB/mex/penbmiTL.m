% TOMLAB PENBMI Solver
%
% function Result = penbmiTL(Prob)
%
% -----------------------------------------------------------------------------
% INPUT:  
...
% Use sdpAssign to define the Prob structure, 
...
%  QP.F       Quadratic coefficients in objective function.
%
%  PriLevOpt  Print Level in penbmiTL and MEX interface
...
% -----------------------------------------------
%
%  SDP          Structure array with Bilinear Matrix Inequality (BMI) data.
% 
%               Please refer to Tomlab User's Guide for information on
%               how to set these fields.
%
% ioptions      (12x1)-vector with integer options.
...
%               Default values are PENBMI defaults and are used 
%               if nothing else is provided.
%
%  ioptions(1)  Set to 0 for PENBMI specified default values
...
%
%  ioptions(4)  Print level: 0=silent, 1,2,3=summary,brief,full.
%               Default: 0
%
...
%               Default: 1.0e-6
%
%  foptions(6)  Lower bound for multipliers.
%               Default: 1.0e-14
%
...
%
%   Inform    PENBMI information parameter.
%
...
%              6  - Memory error.
%              7  - Unknown error, please contact support@tomlab.biz.
%
...
%
%   Solver    Name of the solver (PENBMI).
...
% -----------------------------------------------------------------------
%
% For a problem description, see bmiAssign.m
%
% -------------------------------------------------------------------------
%
% Kenneth Holmstrom, Tomlab Optimization Inc, E-mail: hkh@tomlab.biz
% Copyright (c) 2003-2005 by Tomlab Optimization Inc., $Release: 4.6.0$
% Written Jan 17, 2003.  Last modified Jan 11, 2005.
%
...
if nargin < 1, error('penbmiTL needs the Prob structure as input');return;end
...
Result.Solver='PENBMI';
Result.SolverAlgorithm='BMI Solver PENBMI 2.0';
...
%
% Define lower and upper bound arrays for PENBMI
%
...
if ioptions(1) == 0
    ...
    % OUTPUT
    if ioptions(4) < 0, ioptions(4) = 1; end % SILENT default
...
if ~isempty(Prob.QP.F) & ~all(Prob.QP.F==0)
    Result.f_0 = Result.f_0 + x_0'*Prob.QP.F*x_0;
end
...
if isempty(p)
    ...
    % Bilinear matrix inequalities cell array
    SDP = DefPar(Prob.PENOPT,'SDP',[]);
    
    % THE CALL
    [x_k, info, Obj, v_k, iresults, fresults] = ...
        penbmi(0.5*Prob.QP.F, c, A, Arhs, x_0, SDP, ioptions, foptions, PriLev);
    
else % if isempty(Prob.PENOPT.p)
    p.x0 = x_0;
...
switch(Inform)
   ...
   case 7 , Text = 'Unknown error, please contact support@tomlab.biz.';
...
if PriLev > 0
    fprintf('\n\n-->-->-->-->-->-->-->-->-->-->');
    fprintf('\nPENBMI solving Problem %d:\n',Prob.P);
    fprintf('-->-->-->-->-->-->-->-->-->-->\n\n');
    fprintf('PENBMI: Inform = %2d, ',Inform)
    fprintf('\n');
    for i = 1:size(Text,1)
        fprintf('%s',Text(i,:))
        fprintf('\n')
...
% MODIFICATION LOG:
%
% 030117 ango Written, based on pensdpTL.m
% 030127 ango Changed LMI(i).Q0 --> LMI(i,1).Q0
% 030128 ango Revised comments
% 030526 ango Fixed potential error in nnzA calculation
% 030701 ango Safeguard against sparse Arhs
% 031107 ango Changes for PENBMI 1.1, foptions, new error codes.
% 040102 hkh  Revision for v4.2, call iniSolve and endSolve
% 041202 hkh  Revise calls to defblbu and StateDef
% 041213 frhe Changed to handle the new LMI/BMI format

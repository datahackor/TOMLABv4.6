...
% Written July 24, 2000. Last modified Aug 19, 2002.
%
...
% xState = double(zeros(n,1));
% xState(find(x_k-xTol <= bl(1:n))) = 1; 
% xState(find(x_k >= bu(1:n)-xTol)) = 2;
...
if m1 > 0
   ...
   %    bState = double(zeros(1,m1));
   %    bState(find(Ax  - bTol <= bl(k+1:k+m1))) = 1;
   %    bState(find(Ax >= bu(k+1:k+m1)-bTol)) = 2;
   %    Result.bState = bState;
...
if ~isempty(c_k)
   ...
   %    cState = double(zeros(1,m2));
   %    cState(find( c_k - cTol <= bl(k+1:k+m2))) = 1;
   %    cState(find( c_k >= bu(k+1:k+m2)-cTol))   = 2;
...
%B = ~((x_k > bl(1:n) + xTol) & (x_k < bu(1:n) - xTol) &  (bl(1:n)~=bu(1:n)));

...
%-----------------------------------------------------------------------
% pdco.m: Primal-Dual Barrier Method for Convex Objectives (19 Nov 2003)
%-----------------------------------------------------------------------
...
% [obj,grad,hess] = pdObj( x       );         provided by user, if not Tomlab
% [obj,grad,hess] = pdObj( x, Prob );         provided by user, if Tomlab
%               y = pdMat( name,mode,m,n,x ); provided by user if pdMat
%                                             is a string, not a matrix
%
% INPUT ARGUMENTS:
% pdObj      may be an explicit n x 1 column vector c,
%            or a string containing the name of a function pdObj.m
...
%            Prob   The Tomlab problem structure, only input if Tomlab is used
%         Examples:
...
% 28 Nov 2003: pdxxxmat: y = Aname'*x; replaced by y = (x'*Aname)';
% 29 Dec 2003: Previous line added to Kenneth's 24 Dec 2003 version.
%              We will now try to keep versions in sync.
%-----------------------------------------------------------------------
...
if PriLev > 0
  ...
  % Decode Fname.
  %---------------------------------------------------------------------
  operator  =  ischar(Fname) | isa(Fname,'function_handle');
  explicitF = ~operator;
  ...
  if explicitF
    ...
    if ischar(Fname)
      fname = Fname;
    else % assume Fname is a function handle
  ...
  if explicitA            % assume Aname is an explicit matrix A.
    ...
    if PriLev > 0
      ...
      fprintf('\n\n')
      fprintf('m        = %8g     n        = %8g      nnz(A)  =%9g', m,n,nnzA)
    end
  else
    if ischar(Aname)
       fname = Aname;
    else  % assume Aname is a function handle
	  fname = func2str(Aname);	 
    end
    if PriLev > 0
...
if PriLev > 0
  ...
  Method    = options.Method;
      % 1=Cholesky    2=QR    3=LSQR (SOL) 41= Sparse LU
      % 4 = Tomlab Tlsqr, special PDCO interface avoiding any callbacks
      % 5 = Tomlab Tlsqr, special interface, with callbacks
      % 6 = Tomlab Tlsqr, atol algorithm implemented in Matlab, with callbacks
      % Option 5 and 6 are slower, and normally not to be used
...
if PriLev > 0
  if Method > 3 & Method < 7 
     fprintf('\n\nMethod   = %8g     ', Method)
     fprintf('\n\nUse Tlsqr - Tomlab LSQR MEX with special PDCO interface\n')
  else
  ...
  if operator
    if any(Method==[3,4,5,6])
      % relax
    elseif Tomlab
      fprintf('\n\nWhen A is an operator, we have to use Method = 4')
      Method = 4;
    else
  ...
  case 4
    solver  = ' Tlsqr';  head3 = 'No callbacks';
  case 5
    solver  = ' Tlsqr';  head3 = 'Callbacks';
  case 6
    solver  = ' Tlsqr';  head3 = 'atol algorithm';
  case 41
    solver  = '    LU';  head3 = '        L       U   res';
  otherwise
    error('Method must be 1, 2, 3, 4, 5, 6 or 41')
  end
  ...
  %else   error('Method must be 1, 2, 3, 4, 5, 6 or 41')
  %end
  
  %---------------------------------------------------------------------
  % Categorize bounds and allow for fixed variables by modifying b.
  %---------------------------------------------------------------------
  [low,upp,fix] = pdxxxbounds( bl,bu,PriLev );
...
if PriLev > 0
  ...
  while ~converged
	...
	% Method=4,5,6.  Use Tomlab Tlsqr (iterative solve) to get dy
	% --------------------------------------------------------------
        rhs    = [ D.*w; r1./d2 ];
        damp   = 0;
        ...
        % HKH - Add options to call Tomlab Tlsqr
        switch Method
        case 5
          [ dy, istop, itncg, outfo ] = ...
              pdT2lsqr( nb,m,'pdTlsqrmat',Aname,rw,rhs,damp, ...
                        atol,btol,conlim,itnlim,show,info );
          if precon, dy = pdDDD3 .* dy; end
        case 6
          [ dy, istop, itncg, outfo ] = ...
              pdT3lsqr( nb,m,'pdTlsqrmat',Aname,rw,rhs,damp, ...
                       atol,btol,conlim,itnlim,show,info );
          if precon, dy = pdDDD3 .* dy; end
        case 3
          [ dy, istop, itncg, outfo ] = ...
        ...
        otherwise % Default use special PDCO LSQR MEX interface in Tomlab
          [ dy, istop, itncg, outfo ] = ...
              pdTlsqr( nb,m,'pdTlsqrmat',Aname,rw,rhs,damp, ...
                       atol,btol,conlim,itnlim,show,info );
          % Done in special version of LSQR: if precon, dy = pdDDD3 .* dy; end
        end
...
if PriLev > 0
  ...
  elseif Tomlab
    [obj,grad,hess] = feval( Fname, x, Prob );
  else
...
function [low,upp,fix] = pdxxxbounds( bl,bu, PriLev )
...
% 28 Nov 2003: y = Aname'*x; replaced by y = (x'*Aname)';
%-----------------------------------------------------------------------

  if ischar(Aname)
    y = feval( Aname, mode, m, n, x );
  else
    if mode==1,  y = Aname*x;  else  y = (x'*Aname)'; end
  end
...
function [ x, istop, itn, outfo ] = ...
   pdT3lsqr( m, n, aprodname, iw, rw, b, damp, ...
              atol, btol, conlim, itnlim, show, info )

% Call Tomlab LSQR, Tlsqr - simplest call
% Iterative atol algorithm implemented in Matlab
...
iter = iter +1;
if iter == 1
   [ x, istop, itn, rNorm, xNorm, StdErr, aNorm, aCond, arnorm ] =  ...
        Tlsqr( m, n, aprodname, iw, rw, b, damp, ...
        atol, btol, conlim, itnlim, show, 0, [] );
else
   [ x, istop, itn, rNorm, xNorm, StdErr, aNorm, aCond, arnorm ] =  ...
        Tlsqr( m, n, aprodname, iw, rw, b, damp, ...
        atol, btol, conlim, itnlim, show, 0, [], x );
end
...
%-----------------------------------------------------------------------
% End private function pdT3lsqr
%-----------------------------------------------------------------------

function [ x, istop, itn, outfo ] = ...
   pdT2lsqr( m, n, aprodname, iw, rw, b, damp, ...
              atol, btol, conlim, itnlim, show, info )

% Call Tomlab LSQR, Tlsqr
% Use special LSQR version that implements atol algorithm 
% But still is using callbacks - takes time!
...
%            WantStdErr, nOut, D, x_0 )
%
% Special LSQR interface has the following parameters
%
% function [ x, iStop, Iter, atolold, atolnew, r3ratio ] =  ...
%     Tlsqr( m, n, Aname, iw, rw, b, damp, aTol, bTol, condLim, MaxIter, ...
%            WantStdErr, nOut, D, x_0, r3norm, atolmin )
%
rw(3)= rw(2);
rw(2)= 1;
...
%-----------------------------------------------------------------------
% End private function pdT2lsqr
%-----------------------------------------------------------------------

function [ x, istop, itn, outfo ] = ...
   pdTlsqr( m, n, aprodname, iw, rw, b, damp, ...
              atol, btol, conlim, itnlim, show, info )

% Call Tomlab LSQR, Tlsqr. Use special PDCO interface to Tlsqr
% atol algorithm implemented in Tlsqr
% No callbacks if iw has A matrix, pdxxxlsqrmat implemented in Tlsqr
...
if ischar(iw)
   ...
   if rw(7) == 1 % Precon true
      [ x, istop, itn, atolold, atolnew, r3ratio ] =  ...
           ...
           atol, btol, conlim, itnlim, show, 0, pdDDD3, [], ...
           info.r3norm, info.atolmin, pdDDD1, pdDDD2);
   else
      [ x, istop, itn, atolold, atolnew, r3ratio ] =  ...
           Tlsqr( m, n, iw, iw, rw, b, damp, ...
           atol, btol, conlim, itnlim, show, 0, [], [], ...

% TOMLAB SPRNLP Sparse Barrier NLP Interface
%
% Use tomRun to call SPRNLP: 
%
%   Result = tomRun('sprnlp',Prob);
%
% -----------------------------------------------------------------------------
%
% SPRNLP implements a sparse barrier method to solve the nonlinear 
...
%
%  options    Structure with options for the SPRNLP solver. The user sets
...
function Result=sprnlpTL(Prob)

if nargin < 1, error('sprnlpTL needs the Prob structure as input'); end

Result=bosnlpTL(Prob, 11);

...
% Written Sept 11, 1998.  Last modified May 28, 2004.
%
...
elseif strcmpi(Solver,'PENBMI')
   ...
   SOLPar(30)=2000;
   SOLPar(10)=1E-10;
elseif strcmpi(Solver,'glbFast')
   SOLPar     = SOLGet('npsol',probType,nObj,nJac,m);
   SOLPar(30) = 10000;
   MaxFunc    = 10000;
   EpsF       = 1E-2;   % Relative tolerance (f(x) - fOpt) <= EpsF * fOpt
elseif strcmpi(Solver,'glcFast')
   SOLPar     = SOLGet('npsol',probType,nObj,nJac,m);
   SOLPar(30) = 10000;
   EpsF       = 1E-2;   % Relative tolerance (f(x) - fOpt) <= EpsF * fOpt
   SOLPar(9)  = 1E-5;   % cTol
...
optParam = struct( ...
  ...
  'eps_f',EpsF, 'eps_absf',-realmax, 'eps_x',SOLPar(10), 'eps_dirg',1E-8, ...
  ...
  'QN_InitMatrix',[], ...
  'splineSmooth', 0.4, 'splineTol', 1E-3, ...
...
% 981110  mbk  Add fields splineSmooth and splineTol

...
%  g(x) >= 0  (linear constraints first, then nonlinear)
%
% nlfunc is used as callback from Schittkowski routines
%
% Kenneth Holmstrom, Tomlab Optimization Inc., E-mail: hkh@tomlab.biz.
% Copyright (c) 2003-2004 by Tomlab Optimization Inc., Sweden. $Release: 4.2.0$
...
if mIN > 0
   ...
   if m2 ~= m
      ...
      elseif isempty(cixUpp)
         ...
         m3 = m2+length(cixLow);
         g(m2+1:m3) = sense*(c(cixLow) - Prob.c_L(cixLow));
         %g(m3+1:m3 + length(cixUpp)) = Prob.b_U(cixUpp) - c(cixUpp,:);

%
% Compute constraint violations, return L1-norm 
% Optionally return all violations in one vector
%
% function [h_L1, h] = consviolation(Result,PriLev);
%
% INPUT:
% 
...
%
% OUTPUT:
% h_L1     L1 of violations, i.e. sum of abs of all violations
% h        Violations in variables x, linear constraints A*x and nonlinear
%          constraints c(x)
%
% Kenneth Holmstrom, Tomlab Optimization Inc, E-mail: hkh@tomlab.biz
% Copyright (c) 2003-2004 by Tomlab Optimization Inc., $Release: 4.2.0$
% Written Dec 4, 2003.   Last modified Mar 24, 2004.
%
...
if nargin < 2
   PriLev = [];
   if nargin < 1
      error('ERROR! No input parameters, call consviolation(Result,PriLev)\n');
   end
end
...
if ~isempty(x_k) 
   ...
   if length(bl)==length(zz) & length(bu)==length(zz) 
      ...
      if nargout > 1
         ...
         h_L1=norm(max(0,bl(1:NN)-zz(1:NN)),1)+norm(max(0,zz(1:NN)-bu(1:NN)),1);
      end
   end

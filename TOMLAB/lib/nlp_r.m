...
%
% To compute r(x) nlp_r calls the routine Prob.USER.r either as 
%            r=feval(Prob.USER.r, x) or
%            r=feval(Prob.USER.r, x, Prob, varargin{:})
% depending on the number of inputs
%
% nlp_r computes a weighted residual, using Prob.LS.weigthType/weightW.
%
...
% rWeight Computed and returned if user calls with two arguments.
%
% Kenneth Holmstrom, Tomlab Optimization Inc, E-mail: hkh@tomlab.biz
% Copyright (c) 1998-2004 by Tomlab Optimization Inc., $Release: 4.3.0$
% Written Oct 10, 1998.  Last modified Apr 27, 2004.
%
...
if isempty(Func)
   ...
   elseif Prob.ADObj == 2
      ...
      if isempty(JPI)
         ...
         ad_rM=length(r);
         JPI=getJPI(Func,ad_rM,N,Prob);
         %JPI=getJPI('admit_rJ',ad_rM,N,Prob,'f');
         save adJPI JPI Prob
         % fprintf('Automatic differentiation using JPI in ADMAT.\n');
      end
      [ad_r ad_J]=evalJ(Func,x(1:N),Prob,ad_rM,JPI);
      r=ad_r;
   end
   if Prob.ADObj == 0
      if p > 2

...
% Written July 5, 1999.  Last modified Apr 15, 2004. 
%
...
if strcmpi(Solver,'LINPROG')
   ...
   if exist('linprog')
      ...
      if xGUI
         reset(cla)
         text(0.1,0.9,'No linprog.m. License for Optimization Toolbox?')
         pause(0.05)
      else
...
elseif strcmpi(Solver,'QUADPROG')
   ...
   if exist('quadprog')
      ...
      if xGUI
         reset(cla)
         text(0.1,0.9,'No quadprog.m. License for Optimization Toolbox?')
         pause(0.05)
      else
...
elseif strcmpi(Solver,'FMINCON')
   ...
   Prob.USERX.f   = Prob.USER.f;
   Prob.USERX.g   = Prob.USER.g;
   Prob.USERX.H   = Prob.USER.H;
   % NOT: Send info about functions to optim_cdc
   % Send info about functions to nlp_cdceq
   Prob.USERX.c   = Prob.USER.c;
   Prob.USERX.dc  = Prob.USER.dc;
   Prob.USERX.d2c = Prob.USER.d2c;
...
elseif strcmpi(Solver,'LSQNONLIN') 
   ...
   %if LargeScale
   %   optstruct=optimset(optstruct,...
   %                      'JacPattern',Prob.JacPattern,...
   %                      'MaxPCGIter',Prob.Large.MaxPCGIter,...
   %                      'PrecondBandWidth',Prob.Large.PrecondBandWidth,...
   %                      'TolPCG',Prob.Large.TolPCG,...
   %                      'TypicalX',Prob.Large.TypicalX,...
   %                      );


   % Send info about functions to ls2_rJ
   Prob.USERX.r   = Prob.USER.r;
   Prob.USERX.J   = Prob.USER.J;
   Prob.USERX.d2r = Prob.USER.d2r;
...
elseif strcmpi(Solver,'FMINSEARCH')
   ...
   if exist('fminsearch')
      ...
      if xnargin('fminsearch') == 6
         z = version;
         if z(1) == '5'
            [x_k, Result.f_k, ExitFlag, out] = fminsearch('nlp_f', ...
                  Prob.x_0, optstruct, Prob.x_L, Prob.x_U, Prob);
         else
            [x_k, Result.f_k, ExitFlag, out] = fminsearch(Prob.USER.f, ...
                  Prob.x_0, optstruct, Prob.x_L, Prob.x_U, Prob);
         end
      else

...
% Written Januari 25, 1998.  Last modified Apr 25, 2004.
...
if isempty(IntVars) 
   Ivar=logical(zeros(n,1));
elseif length(IntVars)==1
   Ivar=logical(zeros(n,1));
   if IntVars > 0
      Ivar(1:min(n,IntVars)) = 1;
   end
elseif length(IntVars) < n
   IntVars = round(IntVars);
   Ivar=logical(zeros(n,1));
   ix = find(IntVars > 0 & IntVars <= n);
   Ivar(IntVars(ix)) = 1;
elseif any(IntVars > 1)
   % IntVars is NOT a zero - one vector
   Ivar=logical(zeros(n,1));
   Ivar(IntVars) = 1;
else
   % IntVars is a zero - one vector
   if ~islogical(IntVars)
      IntVars = round(IntVars);
   end
   Ivar=logical(IntVars);
end
...
x_U = Prob.x_U(:);
if MIP 
   x_L(Ivar) = round(x_L(Ivar)+0.49999);
   x_U(Ivar) = round(x_U(Ivar)-0.49999);
end
if issparse(Prob.A)
   SPARSE=1;
   nnzA = nnz(Prob.A);
...
% Call subroutines
[A, b, EqCon, x_L, x_U] = clean(A, b, EqCon, x_L, x_U, Ivar, PriLev);
[A, b, EqCon, mkspflag] = mksp(A, b, EqCon, Ivar);
if mkspflag
   [A, b, EqCon, x_L, x_U] = clean(A, b, EqCon, x_L, x_U, Ivar, PriLev);
end
...
%******************************* CLEAN ******************************
function [A, b, EqCon, x_L, x_U] = clean(A, b, EqCon, x_L, x_U, Ivar, PriLev)
...
while change & feasible 
   ...
   change = 0;
   [A, b, EqCon] = emptyrow(A, b, EqCon, Ivar, PriLev);
   [A, b, EqCon, x_L, x_U, change] = r_rw_sng(A, b, EqCon,...
    x_L, x_U, change, Ivar, PriLev);
   [A, b, EqCon] = emptyrow(A, b, EqCon, Ivar, PriLev);
...
%****************************** EMPTYROW ******************************
function [A, b, EqCon] = emptyrow(A, b, EqCon, Ivar, PriLev)
         ...
         if ~EqCon(i) % Inequality constraint (<=)
            ...
            if A(i,j) > 0 % Bound is a upper bound
               if bound < x_L(j) % Infeasible
                  ...
                  if x_U(j) > bound
                     x_U(j) = bound;
                     if Ivar(j), x_U(j) = round(x_U(j) - 0.49999); end
                     change = 1;
                  end                    
               end
               ...
               if bound > x_U(j) % Infeasible
                  ...
                  if x_L(j) < bound
                     x_L(j) = bound;
                     if Ivar(j), x_L(j) = round(x_L(j) + 0.49999); end
                     change = 1;
                  end
               end
            end
...
%****************************** MKSP ******************************
function [A,b,EqCon,mkspflag] = mksp(A, b, EqCon, Ivar)

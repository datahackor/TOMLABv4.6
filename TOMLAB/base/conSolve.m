...
%
% Schittkowski SQP:
% Solver.Alg  NumDiff   ADObj  isempty(USER.H)   Hessian computation
...
%     1        any       -1,2        any         Automatic differentiation
%     2          0         0         any         BFGS
%     2        ~=0         0         any         BFGS, numerical gradient g
%     2        any       -1,2        any         BFGS, automatic diff gradient
%
% Han-Powell SQP:
% Solver.Alg  NumDiff    ADObj isempty(USER.H)   Hessian computation
...
% Copyright (c) 1995-2004 by Tomlab Optimization Inc., $Release: 4.5.0$
% Written June 5, 1995.   Last modified Nov 26, 2004.
%
...
%eps_absf = optParam.eps_absf; % Absolute convergence tolerance in function
...
%   CHECK CONVERGENCE HERE
   ...
   if f_k <= optParam.eps_absf      % Stop criteria 2
   ...
   if (Inform < 100 | Inform==102) & stop
      ...
      %    j=n+mA+i;
      %    if cEqual(i)
      %       cErr(i)=-abs(c_k(i)-Prob.c_L(i));
      %       ceq=ceq+(abs(cErr(i)) > cTol*max(1,c_k(i))));
      %       if (abs(cErr(i)) > cTol*max(1,abs(c_k(i)))),pen(j)=pen(j)*100;end
      %    elseif c_k(i)-Prob.c_L(i) < cTol*max(1,abs(c_k(i)))
      %       cErr(i)=c_k(i)-Prob.c_L(i);
      %       cineq=cineq+ (abs(cErr(i)) > cTol*max(1,abs(c_k(i))));
      %       if abs(cErr(i)) > cTol*max(1,abs(c_k(i)), pen(j)=pen(j)*100; end
      %    elseif Prob.c_U(i)-c_k(i) < cTol
      %       cErr(i)=Prob.c_U(i)-c_k(i);
      %       cineq=cineq+ (abs(cErr(i)) > cTol*max(1,abs(c_k(i))));
      %       if abs(cErr(i)) > cTol*max(1,abs(c_k(i))), pen(j)=pen(j)*100; end
      %    end
      %end
      ...
      %for i=1:mA
      %    j=n+i;
      %    if Prob.b_L(i)==Prob.b_U(i)
      %       bErr(i)=-abs(Ax(i)-Prob.b_L(i));
      %       beq=beq+(abs(bErr(i)) > bTol);
      %       if (abs(bErr(i)) > bTol),pen(j)=pen(j)*100; end
      %    elseif Ax(i)-Prob.b_L(i) < bTol
      %       bErr(i)=Ax(i)-Prob.b_L(i);
      %       bineq=bineq+ (abs(bErr(i)) > bTol);
      %       if abs(bErr(i)) > bTol, pen(j)=pen(j)*100; end
      %    elseif Prob.b_U(i)-Ax(i) < bTol
      %       bErr(i)=Prob.b_U(i)-Ax(i);
      %       bineq=bineq+ (abs(bErr(i)) > bTol);
      %       if abs(bErr(i)) > bTol, pen(j)=pen(j)*100; end
      %    end
      %end
      ...
      if (any([cineq,ceq,beq,bineq] > 0)) & ...
          (Iter < MaxIter & f_k >= optParam.eps_absf)
   ...
   if stop            % Show final results
      if PriLev >= 1  
         switch Inform
            ...
            case 102
               ...
               disp('*** Restart with lower optParam.eps_absf ');
      ...
      if PriLev >= 5  
         ...
         xprinte(v_k,'v_k:');
         if Alg==1 | Alg==3
            printmat(B_k(1:min(n,MAX_x),1:min(n,MAX_x)),'The Hessian H_k:')
         end
      end
      Result.x_k=x_k;
      Result.v_k=v_k;
...
if DEBUG 
disp('EXPANDED QP: x_L x_U ; F ; c ; A ; b_L b_U')
[ProbQP.x_L ProbQP.x_U]
ProbQP.QP.F
ProbQP.QP.c
ProbQP.A
[ProbQP.b_L ProbQP.b_U]
pause
end

%r_k=ProbQP.A*ProbQP.x_0-ProbQP.b_L
%r_k2=-(ProbQP.A*ProbQP.x_0-ProbQP.b_U)
%find(r_k <0)
%find(r_k2 <0)
%pause
        
         %ResultQP = SolveQP(ProbQP);
...
%r_k=ProbQP.A*p-ProbQP.b_L
%r_k2=-(ProbQP.A*p-ProbQP.b_U)
%find(r_k <0)
%find(r_k2 <0)
   ...
   if norm(z) > eps_x
      ...
      if norm(y) > eps_g
         ...
         if PriLev > 3
            ...
            printmat(B_k(1:min(n,MAX_x),1:min(n,MAX_x)),'B_k:')
         end
...
if Alg==1 & Prob.NumDiff==0 & (~isempty(Prob.USER.H) | any(Prob.ADObj == [-1,2]))
...
elseif Alg==3 & Prob.NumDiff==0 & ...
       (~isempty(Prob.USER.H) | any(Prob.ADObj == [-1,2]))
...
elseif Alg==4 
   ...
   % Default algorithm
   if Prob.NumDiff==0 & (~isempty(Prob.USER.H) | any(Prob.ADObj == [-1,2]))
      SolvAlg='Schittkowski SQP algorithm, analytic Hessian';
      merit=0; % merit == 0 ==> Augmented Lagrangian
      minpen=0;   % HKH special. Emergency increase of the penalities
      Alg = 1;
   %elseif isempty(Prob.USER.g) | Prob.NumDiff > 0
   else
      SolvAlg='Schittkowski SQP algorithm with BFGS update';
      merit=0; % merit == 0 ==> Augmented Lagrangian
      minpen=0;   % HKH special. Emergency increase of the penalities
...
%if DEBUG 
%disp('QP: x_L x_U ; F ; c ; A ; b_L b_U')
%[ProbQP.x_L ProbQP.x_U]
%ProbQP.QP.F
%ProbQP.QP.c
%%[ProbQP.b_L ProbQP.A ProbQP.b_U]
%end

   if M > n
      ProbQP.b_L=bl(n+1:M)-z;
      ProbQP.b_U=bu(n+1:M)-z;
...
%pause
      ...
      %ResultQP = SolveQP(ProbQP);
      ResultQP = tomSolve(SolverQP,ProbQP);
      %ResultQP = tomRun(SolverQP,ProbQP,2);
...
if DEBUG
fp_0
disp('accept the max iter solution');
pause
end
         else
if DEBUG
fp_0
disp('DO NOT accept the max iter solution');
pause
end
         end
      end

...
% Written Nov 7, 1998.    Last modified Jan 17, 2005.

function ResultLP = lpSolve(Prob)

if nargin < 1
   error('lpSolve needs input structure Prob');
end

solvType=checkType('lp');

Prob=ProbCheck(Prob,'lpSolve',solvType);
...
if length(b_U)~=m
   ...
   error('Illegal input to lpSolve');
end
...
if any( bl-bu > rTol .* max(1,abs(bl+bu)))
   ...
   ResultLP.QP.B=[];
   disp('lpSolve detected infeasible problem');
   if PriLev > 1
      disp('lpsolve detected infeasible problem');
      [bl bu]
      if wait, pause; end
...
if mA==0
   ...
   elseif Stop==2
      if PriLev > 0
         disp('lpSolve: Illegal initial logical basis B given as input')
         xprinti(B(1:mP),'B:');
      ...
      return
   elseif Stop==3
      disp('Illegal initial x to lpSolve')
      xprinte(xScale(x, Ascale, D2, mP),'x:');
      ResultLP.x_k=xScale(x, Ascale, D2, n0);
      ResultLP.QP.B=B(1:n0);
...
%pause
   ...
   elseif Phase==1
      if SolveFP
         ...
         if PriLev > 0
            fprintf('\n=======******-------------------------------------')
            fprintf('-----------------------\n')
            fprintf('lpSolve: Phase 1 converged in %d iterations\n',Iter)
            fprintf('=======******-------------------------------------')
            fprintf('-----------------------\n')
            if PriLev > 1
   ...
   if minCost(1) > -eps_f & Phase==1
      if Pen > 1E12 | minCost(1) >= -eps
         ...
         if PriLev > 0
            ...
            disp('=======================================');
            disp('lpSolve: NO FEASIBLE SOLUTION FOUND!!!!');
            disp('=======================================');
   ...
   if Cycle > n
      if PriLev >= 0
         fprintf('lpSolve: Cycling! Iteration %d\n',Iter)
      end
      if IndexRule==1 | IndexRule==3
         useBland=0;
         if PriLev > 0
            disp('lpSolve: Bland''s rule does not work!')
         end
      else
...
if length(iB) > m
   disp('lpSolve: Error! Number of variables in basis are too large');
   ...
   if pHat ~= 0
      if (mod(Iter,qrFull)==0 & qrFull > 0) | (doQR > 0 & sinceQR > 10)  
         ...
         k=find(q==iB);
         if isempty(k)
            error('lpSolve: Fatal Error. Empty variable');
         end

         [Q,R,P,pRank,maxR,absR] = InsertQR(Q, R, P, pRank, k, A(:,q), epsRank);

if any(sort(P)~=[1:m]')
   disp('lpSolve: Serious P error');
...
if PriLev >= 0
   fprintf('\nlpSolve: TOO MANY ITERATIONS --- ITER =%7.0f\n',Iter)
   fprintf('\nEither cycling or some numerical problems occured\n')
...
elseif abs(x(p) - x_U(p)) <= rTol(p) * max(1,abs(x(p)))
   ...
   if PriLev >= 0
      disp('lpSolve: Variable is NOT close to bound')
...
if any(sort(P)~=[1:length(P)]') 
   disp('lpSolve (FullRank): Serious P error');
   disp('Bad input data, or some logical program error.');
   %size(R)
   %size(E)
   m
...
if EMPTY & isempty(B)
   ...
   if m0 > m
      ...
      fprintf('lpSolve: Too many degrees of freedom.')
...
% ------------------------------------------------------------------------
   ...
   if ~isempty(Prob.QP.HotFile)
      ...
      if PriLev > 2
         fprintf('lpSolve: Load Warm Start Basis File %s',Prob.QP.HotFile)
         fprintf('\n')
      end
   else
      if PriLev > 2
         fprintf('lpSolve: Load Warm Start Basis From Prob structure')
         fprintf('\n')
      end
      x = Prob.QP.Hot.x;
      B = Prob.QP.Hot.B;

...
%
% INPUT PARAMETERS   (Use tomMenu.m or tomGUI.m for interactive run)
%
...
%              If Prob.ADObj = 2,  ADMAT automatic differentiation is used
...
% Copyright (c) 1992-2004 by Tomlab Optimization Inc., $Release: 4.4.0$
% Written Dec 2, 1992.    Last modified Oct 18, 2004.
%
...
if Alg == 0
   if Prob.NumDiff == 0 & ~isempty(Prob.USER.g)
...
eps_absf = optParam.eps_absf; % Absolute convergence tol on function value
...
%
   ...
   if f_k <= eps_absf   % Stop criteria 2
      if PriLev >= 1
         disp('*** STOP 2! Function value below optParam.eps_absf   ***');
...
%
   ...
   elseif method == 3		% Matlab built in inversion routine
      if Alg==1
         [Hrows Hcols]=size(H_k(idx,idx)); % Add dummy column, gives QR-decomp.
         p = -([full(H_k(idx,idx)),zeros(Hrows,1)] \ g_k(idx));% Matlab does QR
      else
         [Hrows Hcols]=size(B_k(idx,idx)); % Add dummy column, gives QR-decomp.
...
%                                      1
   ...
   if fp_0 >= 0
      eig_step=1;
      if PriLev >= 1
         disp('ALARM: No descent direction.')
      ...
      for j=1:length(f_pp);
          ...
          if alphaMax <= 1E-7
             if PriLev > -1
                fprintf('No move in eig dir possible!! Local minimum. ');
                fprintf('alphaMax=%15.7e. Trouble with var %d\n',alphaMax,xvar);
             end
          end
          if PriLev >=3
...
% 990306  hkh   Test on USER.H instead of p_H, to see if Hessian defined
...
% 000919  hkh   Check on Prob.USER.g if user has not given derivative

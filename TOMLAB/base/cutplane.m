...
% Copyright (c) 1994-2004 by Tomlab Optimization Inc., Sweden. $Release: 4.6.0$
% Written Nov 28, 1994.   Last modified Dec 22, 2004.
%
...
PriLevOpt = Prob.PriLevOpt;  % Print level in lpSolve and DualSolve
%PriLev=2
%PriLevOpt=2
...
if PriLev > 2 & PriLevOpt >= 1
   disp('=+=+=+=+=+=+=+=')
   fprintf('=== cutplane:    LP relaxation. Call lpSolve.\n')
   disp('=+=+=+=+=+=+=+=')
end
...
%ProbLP.optParam  = optParamDef(SolverLP,8);
%ProbDLP.optParam = optParamDef(SolverDLP,8);
...
if mU > 0
   ...
   if ~isempty(VarWht)
      maxW = max(VarWht(~isinf(VarWht)));
      VarWht = [VarWht;maxW*ones(mU,1)];
   end
...
%fprintf('l-iCand %d, l-OldC %d',length(irBest),length(OldC))
   ...
   if IterPrint & PriLev < 2
      ...
      fprintf('Nonint %d/%d ',nReal,nIvar)
...
if length(iB)~=pRank
   ...
   ResultLP = tomSolve(SolverDLP,ProbDLP);
   ...
   fDP = ResultLP.f_k;
   B   = ResultLP.QP.B;
%B   = ResultLP.QP.BFull(1:n);
%disp('efter')
%length(B)
%length(ResultLP.QP.BFull)
%pause
...
if ExitFlag==0 & abs(cTx-fDP)/max(1,abs(cTx)) > 1E-12 & DEBUG

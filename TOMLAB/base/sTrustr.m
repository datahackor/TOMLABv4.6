...
% Copyright (c) 1996-2004 by Tomlab Optimization Inc., Sweden. $Release: 4.2.0$
% Written Sept 12, 1996.  Last modified Jan 25, 2004.
%
...
fLow   =   optParam.eps_absf; % Lower bound on function value
...
if isempty(Prob.SolverQP)
   ...
   % TOMLAB qpSolve seems to work best, at least on mgh_prob
   % QPOPT is crashing for Brown badly scaled, when D is big
   % Use qld for speed, and not qpopt

   if strcmpi('qpopt',SolverQP)
      SolverQP = 'qld';
   end
...
if any(zErr > zTol .* max(1,abs(z))) 
   ...
   ResultLP = tomSolve(SolverFP,ProbLP);
...
%
   ...
   if maxgProj * max(abs(x_k),size_x) <= eps_g * max(abs(f_k),size_f) ...
   ...
   if f_k <= fLow-10*eps       % Stop criteria 2
      if PriLev >= 1
         disp('*** STOP 2! Function value below given estimate   ***');
         disp('*** Restart with lower fLow if min not reached ***');
      end
   ...
   if max(abs(D)) > 1E15
      ...
      if Inform >0 & Inform < 100, flag=0; else, flag=1; Inform=103; end
      stop = 1;
   end
...
if alg==0 & Prob.NumDiff==0 & (~isempty(userH) | any(Prob.ADObj == [-1 2]))

...
% Copyright (c) 1994-2004 by Tomlab Optimization Inc., $Release: 4.6.0$
% Written Apr 20, 1994.   Last modified Dec 2 2004.
%
...
if isempty(Prob.SolverFP)
   ...
   %if strcmpi(SolverFP,'qld')
   %   % Set lpSolve, using qld gives failure on cls_prob-8, DAS 4 - scales???
   %   SolverFP='lpSolve';
   %end
...
if DEBUG 
   %disp('bl g bu')
   xprint(x,'New x: ');
   printmat([bl g bu],'bl x;Ax bu',120)
   if wait, pause; end
end
...
if NonFeas | FP 
   ...
   if ExitFlag > 0  %  Phase 1 failed
      ...
      ResultQP = endSolve(Prob,ResultQP);
      return
   end
...
%end
   ...
   if p_sq >= 1E-12 | LP % STEP in subspace of Working Set.
...
ResultLP=tomSolve(SolverFP,ProbQP);
...
% 1 = QPOPT, 2 = MINOS, 3 = lpSolve

n= length(x1);

if 1
% Run QPOPT to test results against lpSolve on FP 
Solver2 = 'qpopt';
% Run MINOS to test results against lpSolve on FP 
% Solver2 = 'lp-minos';

Solver2

ResultLP2=tomSolve(Solver2,ProbQP);
...
if any(abs(x1-x2) > 100*eps) & (ExitFlag == 0 | ExitFlag2==0)
   fprintf('FP: diff between %s and lpSolve\n',Solver2);
   xprinte(x1,'x1: ');
   xprinte(x2,'x2: ');
...
switch  ExitFlag
   ...
   case 3
     Text = 'Rank problems';
   case 4
     Text = 'No feasible point found with lpSolve';
   case 10
...
% 981111  hkh  Change FP solution to call lpSolve
...
% 990827  hkh  Big revision for v2.0 using new lpSolve features 

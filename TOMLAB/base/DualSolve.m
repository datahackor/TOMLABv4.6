...
% Handbooks in Operations Research and Management Science, ChapterII: 
...
% Written Nov 10, 1994.   Last modified Jan 17, 2005.
...
%pause
      ...
      % We give up and call lpSolve instead
      %disp('**** NOT FULL RANK, call lpSolve **************')
      Prob.x_0=[];
      Prob.B  =[];
      ResultDLP=lpSolve(Prob);
      return
   end
...
if any(cHat(iN) < 0)
   if any(cHat(iN) < -eps_f)
      ...
      % EMERGENCY! Call lpSolve
      if PriLev > 0
         disp('*** NOT DUAL FEASIBLE! Call lpSolve instead ***')
      end
      Prob.x_0=[];
      Prob.B  =[];
      ResultDLP=lpSolve(Prob);
...
%end
   ...
   if dErr > 1E-8*bNorm
      if PriLev > 1 | dErr > 0.1*bNorm
         ...
         %% We give up and call lpSolve instead
         %disp('**** Numerical ill-conditioning, call lpSolve **************')
         %Prob.x_0=[];
         %Prob.B  =[];
         %ResultDLP=lpSolve(Prob);
         %return

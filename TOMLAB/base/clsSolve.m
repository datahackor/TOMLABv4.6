...
%                      pp 108-129 jan 1994.
% Prob.Solver.Alg = 4  The Gauss-Newton method 
% Prob.Solver.Alg = 5  Wang, Li, Qi Structured MBFGS method
% Prob.Solver.Alg = 6  Wang, Li, Qi Structured BFGS method
%
% Prob.Solver.Alg = 0, empty or other number, gives the Fletcher-Xu algorithm
%
...
%
% INPUT PARAMETERS (Use tomMenu.m for interactive run/ tomRun as driver routine)
%
...
% Written Jan 15, 1998.   Last modified Jan 21, 2005.
%
...
% NOT USED YET:
%   USER.d2r:The routine to compute the 2nd derivative wrt r, as a string
...
LineParam.fLowBnd = max(0,Prob.LineParam.fLowBnd); 
...
%ONLY USED if NONLINEAR CONSTRAINTS: cTol =  optParam.cTol;    
...
eps_absf = max(0,optParam.eps_absf); % Absolute convergence tol in function
...
[x_k, me, mc, mi, LCflag, A, b, W_kset]=leqCheck(Prob, ProbQP, ...
...
% Check if variables near bound and set up active variable indicator set
   ...
   if changed | k==0
      ...
      if k==0
         ...
         JJ = J_k'*J_k;
         %HKH
         %A_k= norm(JJ,2)*eye(Prob.N,Prob.N);
         if alg>=5
            A_k= norm(JJ,'fro')*speye(Prob.N,Prob.N);
            %A_k= norm(JJ)*eye(Prob.N,Prob.N);
         end
...
%
   ...
   if ~(release | releasecon)% | fRed0cnt > LowIts % Check convergence criteria
      ...
      if all(lambda >=-bTol | fixbnd) 
         ...
         if f_k <= eps_absf*size_f            % Convergence criteria 3
            if PriLev >= 1
               disp('*** Convergence 3, Function value close to zero ***');
            end
            flag=0;
            Inform = Inform + 4;
            stop = 1;
         ...
         if f_red < 1E-30*max(1,f_k)   % Convergence criteria 5
            if PriLev >= 1
...
%
   ...
   if k >= MaxIter		% Stop criteria 1
      ...
      if Inform >0 & Inform < 100, flag=0; else, flag=1; Inform=101; end
      stop = 1;
   end

   if  f_k < eps_absf     % Stop criteria 2
      if PriLev >= 1
         disp('*** STOP 2, Function value below given estimate ***');
         disp('*** Restart with lower Prob.optParam.eps_absf ***');
         disp('*** if minimum not reached ***');
      end
      Result.ExitText=str2mat('Function value below AbsF' ...
             ,'Restart with lower Prob.optParam.eps_absf' ...
             ,'if minimum not reached');
      if Inform >0 & Inform < 100, flag=0; else, flag=1; Inform=102; end
      stop = 1;
   end
   ...
   if stop			% Show results before return
      ...
      switch  Inform
         ...
         case 18
           Text = 'Small gradient, relative f(x) reduction, close points';
         case 32
...
%*******************************************************************************   
      ...
      if alphaMax <= 1E-14   % % Too small value. Can not take any step
         ...
         if LineAlg <= 1
            ...
            LineResult=Armijo(x_k,p_full,f_k,g_k,agFac,sigma,Prob,varargin{:});
         end
...
%alpha
%pause
%if alpha > 0 & alpha > 1E-10*alphaMax
%alpha
%disp('USUUS')
%pause
%end
...
%g_k
%   p_full'*g_k
%   r_z = nlp_r(x_k, Prob, varargin{:});     % Function value
%fprintf('%15.13f %30.20f %30.20f\n',0,0.5*r_z'*r_z,0.5*r_z'*r_z)
%   aaa=10*alphaMax;
%   for ii=0:20
%       aaa=aaa/10;
%       xzz=x_k-aaa*p_full;
%       r_z = nlp_r(xzz, Prob, varargin{:});     % Function value
%       xzz=x_k+aaa*p_full;
%       r_z2 = nlp_r(xzz, Prob, varargin{:});     % Function value
%fprintf('%15.13f %30.20f %30.20f\n',aaa, 0.5*r_z'*r_z, 0.5*r_z2'*r_z2)
%   end
%end
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      if alg==4
         ...
         if alpha > 100 * eps & pNorm > 0 % Check if we moved from x_k
            [B_k, GN, A_k] = SafeUpdate( alg, alpha, p_full, eps_x, f_k, ...
                                        f_km1, J_k, J_km1, r_k, B_k, JJ, ...
                                        g_k, g_km1, A_k, PriLev, k, pCond,GN);
         end
...
function [B_k, GN, A_k] = SafeUpdate(alg, alpha, p_full, eps_x, f_k, f_km1,...
          J_k, J_km1, r_k, B_k, JJ, g_k, g_km1, A_k, PriLev, k, pCond,GN)
% =========================================
...
if alg == 3 % Huschens TSSM algorithm. Always BFGS type of update.
   ...
   %   %sum(Bp~=B_k)
   %   printmat(Bp-B_k,'Bp-B_k:') % Pretty equal, about 1E-10 error
   %   pause
   %end
...
if alg == 5 % Wang, Li, Qi Structured MBFGS method
   z     = alpha*p_full; % Check if we have converged (or short alpha step)
   n     = length(z);
   gNorm = norm(g_km1);
   %gNorm = norm(g_k); % Use old or new gradient
   JJz   = JJ*z;
   yC    = (J_k-J_km1)'*r_k;
   gamma = JJz + yC;
   %gamma = JJz + (J_k-J_km1)'*r_k;
   zz    = z'*z;
   if zz == 0 | gNorm == 0
      tk = 1;
   else
      tk = 1 + max(-gamma'*z/zz,0)/gNorm;
   end
   %y = JJ*z+(J_k'-J_km1')*r_k + tk*gNorm*z;
   y = gamma + tk*gNorm*z;
   ...
   if GN == 0
      % Modify A_k to A_k^s
      A_ks = A_k + J_km1'*J_km1 - JJ;
   else
      %A_ks = norm(r_k)*eye(n,n);
      A_ks = B_k + J_km1'*J_km1 - JJ;
      %A_ks = A_k + J_km1'*J_km1 - JJ;
   end
   %A_ks = B_k + J_km1'*J_km1 - JJ;
   % Check update
   ...
   %A_k = A_ks + secUpdat(z,y,A_ks,v); % Secant update
   %A_k = A_ks + secUpdat(z,gamma,A_ks,v); % Secant update

   % Check update
   BB = B_k - Bz*Bz'/zBz + (y/zy)*y';
   %B1 = B_k - Bz*Bz'/zBz + (y/zy)*y';
   %B2 = B_k + secUpdat(z,y,B_k,v); % Secant update

   if (f_km1-f_k) >= 0.2*f_km1 | norm(z) <= eps_x
      GN  = 1;
      B_k = JJ + norm(r_k)*speye(n,n);
      %B_k = JJ + norm(r_k,'fro')*eye(n,n);
      A_k = norm(r_k)*speye(n,n);
      %A_k = B_k;
   else % MBFGS update. 
      GN  = 0;
      B_k = JJ + A_k;
...
if alg == 6 % Wang, Li, Qi Structured BFGS method
   z     = alpha*p_full; % Check if we have converged (or short alpha step)
   n     = length(z);
   JJz   = JJ*z;
   yC    = JJz + (J_k-J_km1)'*r_k;
   gNorm = norm(g_km1);
   %gNorm = norm(g_k); % Use old or new gradient
   ...
   % Check update
   B1 = B_k - Bz*Bz'/zBz + (y/zy)*y';
   BB = B_k + secUpdat(z,y,B_k,v); % Secant update
...
elseif alg==5
   SolvAlg='Wang, Li, Qi MBFGS method';
elseif alg==6
   SolvAlg='Wang, Li, Qi BFGS method';
else
   % Default algorithm
   alg=1;
   SolvAlg='Fletcher-Xu Hybrid Method. Modified Gauss-Newton - BFGS';
...
% =========================================
function [x_k, me, mc, mi, LCflag, A, b, W_kset]=leqCheck(Prob, ProbQP, ...
         SolverQP, x_k, b_L, b_U, x_L, x_U, bEqual, Ax, bTol, PriLev)
% =========================================
...
if mc > 0 % If any linear constraints
   ...
   if any( Ax + bTol < b_L | Ax - bTol > b_U ) % Not feasible
      ...
      if n > 1000
         ProbQP.QP.F = spdiags(1./max(1E-3,x_k.^2),0,n,n);
      else
         ProbQP.QP.F = diag(1./max(1E-3,x_k.^2));
      end
      ProbQP.QP.c = -ProbQP.QP.F'*x_k;         
      %ProbQP.QP.F = eye(n,n);
      %ProbQP.QP.c = -x_k;         
      ProbQP.A    = Prob.A;
      ProbQP.b_L  = b_L;  ProbQP.b_U  = b_U;
      ProbQP.x_L  = x_L;  ProbQP.x_U  = x_U;
      ProbQP.x_0  = zeros(n,1);
      ProbQP.mLin = size(ProbQP.A,1);

% NOTE! Minos and QPOPT have problems with ill-conditioned problems
% Better use qpSolve - lpSolve for phase1

      ResultQP = tomSolve(SolverQP,ProbQP);
...
%ResultQP.Inform
      ...
      %if ExitFlag==3 % Try Phase 1 LP
      %   ProbQP.QP.F=[];
      %   ProbQP.QP.c=[];
      %   ProbQP.SolverFP=2;
      %   ResultQP = SolveFP(ProbQP);
      %   x_k=ResultQP.x_k;
      %   ExitFlag=ResultQP.ExitFlag
      %end
      %u_k=ResultQP.v_k;
...
% Copyright (c) 1997-2004 by Tomlab Optimization Inc., Sweden. $Release: 4.0.0$
...
% ==========================================================
function LineResult=Armijo(x_0,p,f_0,g_0,rho,sigma,Prob,varargin);
% ==========================================================
%
% Kenneth Holmstrom, Tomlab Optimization Inc., E-mail: hkh@tomlab.biz.
% Copyright (c) 1997-2004 by Tomlab Optimization Inc., Sweden. $Release: 4.0.0$
...
% 990221  hkh   Check if Prob.USER.d2r is defined, before computing 2nd der
...
% 030525  hkh   Addition of Structured BFGS method by Wang-Li-Qi

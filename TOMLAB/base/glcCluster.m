...
%
% glcCluster is a hybrid algorithm, that is using glcFast (a DIRECT algorithm)
% for global search, a clustering algorithm to find suitable initial
% points, and then local search from these points. 
% The 4th step is to run glcFast once again.
% If glcFast improves the best point, a local search is finally made with
% the new best point(s) as starting points.
...
%   maxDistmin The minimal distance used for clustering, default 0.05
...
%
%    Prob.GO.maxFunc1 = 1000; % Number of initial function evals to glcFast
%    Prob.GO.maxFunc2 = 200;  % Number of final   function evals to glcFast
...
% Copyright (c) 2001-2004 by Tomlab Optimization Inc., $Release: 4.6.0$
% Written Nov 3, 2001.    Last modified Nov 29, 2004.
%
...
if isfield(Prob.GO,'localTry')
   localTry   = Prob.GO.localTry;
else
   localTry = [];
end
if isempty(localTry), localTry = 30; end
...
if isempty(maxDistmin), maxDistmin = 0.05; end

Result                 = ResultDef(Prob);
Result.Solver          = 'glcCluster';
Result.SolverAlgorithm = ...
      ['Constrained DIRECT-Cluster-Local Search with ' localSolver];
...
if sum(ixVars) == n
   ...
   fprintf('            ');
   fprintf('Call glcFast, glcSolve, or some other IP solver instead\n');
...
%hold off    % Plotting is on if this line is used
if Prob.WarmStart == 1
   load(DIRECTFILE,'F');
   fPrev = length(F);
else
...
if ~feasible & totfEval >= mFunc
   ...
   Result.SolverAlgorithm = ...
      ['Constrained DIRECT-Cluster-Local Search with ' localSolver];
   Result.MinorIter= r.Iter;      % Number of iterations in glcFast/glcSolve
...
if PriLev > 2 
   fprintf('                      ')
   fprintf('Starting cluster procedure\n'); 
end

Prob.WarmStart = 0;  % No WarmStart in the local solvers

% C and F could have been returned directly from glcFast/glcSolve

load(DIRECTFILE,'C','glcfMin','F','ignoreIdx','fMinIdx','fMinEQ'); 
if PriLev > 0
   fprintf('glcCluster Phase 1: ')
...
% mean( minl1) is the average distance any point has to its closest neighbour
% v is the set of points with that wont be clustered
v=find(minl1>=mean(minl1));

% v2 is the set of points being considered for clustering
v2=find(minl1<mean(minl1));

%plot(C(1,v),C(2,v),'s') % Plotting
%pause(0.001)            % More plotting

C2=C(:,v2);              % Constructing a temporary group, better to use index
F2=F(v2);
...
% In that way not too big or too small values will be used.
% Note. A test in see4.m showed that more work is needed on this distance
% Although the see4  way of testing was not bad!
if PriLev > 1
   fprintf('                      ')
   fprintf('maxDist %f\n',maxDist);
end
...
if length(v4) >= 1
   for i=1:length(v4)
       ...
       if i==1
          %plot( C2(1,v3(1:v4(i)-1) ),C2(2,v3(1:v4(i)-1)),[col 'o'])   %PLOTT
          F3 = F2(v3(1:v4(i)-1));
          C3 = C2(:,v3(1:v4(i)-1));
          [Fv(i) idxMt] = min(F3);
          c_k=[c_k C3(:,idxMt(1))];
       else
          %plot(C2(1,v3(v4(i-1)+1:v4(i)-1)),C2(2,v3(v4(i-1)+1:v4(i)-1)),[col 'o'])  %PLOTT
          F3=F2(v3(v4(i-1)+1:v4(i)-1) );
          C3=C2(:,v3(v4(i-1)+1:v4(i)-1) );
          [Fv(i) idxMt] = min(F3);
          c_k=[c_k C3(:,idxMt(1))]; 
       end
       
   end
   %plot(C2(1,v3(v4(length(v4))+1:length(v3))),C2(2,v3(v4(length(v4))+1:length(v3))),['yo'])  %PLOTT
   
   F3=F2(v3(v4(length(v4))+1:length(v3)));
   C3=C2(:,v3(v4(length(v4))+1:length(v3)));
   [Fv(length(Fv)) idxMt] = min(F3);
   c_k = [c_k C3(:,idxMt(1))];
else
   %plot(C2(1,v3),C2(2,v3),'ro')           %PLOTT
   F3 = F2(v3);
   C3 = C2(:,v3);
   [Fv idxMt] = min(F3);
   c_k = C3(:,idxMt(1));
...
if PriLev > 2
   fprintf('                      ')
   fprintf('glcCluster: Number of clusters %d\n',nPnt)
end
...
[Fv, idx] = sort(Fv); % sorting Fv.
x_k = x_k(:,idx);     % sorting x_k accordingly.
...
if localTry <= 0
   ...
   Result = r;
   Result.Solver          = 'glcCluster';
   Result.SolverAlgorithm = 'Constrained DIRECT / Cluster / NO Local Search';
   Result.Cluster.maxDist = maxDist;
   Result.Cluster.minDist = minl;
   Result.Cluster.x_k     = x_k;
   Result.Cluster.f_k     = Fv;
...
if PriLev > 2 
   fprintf('                      ')
   fprintf('Will now start local search\n'); 
end
...
if PriLev > 0
   fprintf('glcCluster Phase 2: ')
   fprintf('Number of initial points from clustering: %d\n',size(x_k,2))
end
   
ixBest = 0;
for i=1:min(localTry,size(x_k,2))
    ProbL.x_0 = x_k(:,i);
    % Should be changed into indexes
    if ~isempty(IntVars)
    ...
    %plot(ct(1,:),ct(2,:),'g^')         % Plotting
    %if ~isempty(idx3)
    %   F(idx3) = rl.f_k;
    %   C(:,idx3) = ct;
    %else
    %   if PriLev > 0, fprintf('The point didnt exist???\n'); end
    %end
    %X    = [X rl.x_k];
    %Floc = [Floc f_k];
    if ~isempty(IntVars)
...
if ixBest > 0
   ...
   if 1 
      ...
      %fMinIdx    = ixMin; % glcFast/glcSolve returns empty x_k then
      ixMin      = fMinIdx(1);
      %ixMin      = 1;
      F(ixMin)   = fMin;
      C(:,ixMin) = CMin;
...
if M > 0 & feasible & Phase5
   if isempty(Result.x_k)
      ...
      if h_L1 > 10*M*ProbL.optParam.cTol
         % Infeasible second glcFast/glcSolve solution
         if PriLev > 0 
            fprintf('                    ');
...
if Phase5
   if glcfMin <= Result.f_k + fMinEQ
      % No improvement by glcFast/glcSolve
      if PriLev > 0 
         fprintf('glcCluster-Phase 4: ')
...
if Phase5
   % glcFast/glcSolve improved the best point
   glcfMin = Result.f_k + fMinEQ;
   x_k     = Result.x_k;
   fMin    = glcfMin;
   xMin    = x_k(:,1);
...
% 040326  hkh  Must test Phase IV result even if no Phase5
